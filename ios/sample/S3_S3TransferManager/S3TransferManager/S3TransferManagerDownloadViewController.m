...
- (IBAction)downloadFile:(id)sender {
    if(self.downloadFileOperation == nil || (self.downloadFileOperation.isFinished && !self.downloadFileOperation.isPaused)){
        ...
        self.downloadFileOperation = [s3TM downloadFile:self.filePath bucket:[Constants transferManagerBucket] key:kKeyForSmallFile];
    }
}

- (IBAction)pauseDownload:(id)sender {
    [s3TM pauseAllTransfers];
}

- (IBAction)resumeDownload:(id)sender {
    // When you resume, the original handle to the S3TransferOperation
    // is no longer valid.  Update with the new handle.
    self.downloadFileOperation = [s3TM resume:self.downloadFileOperation requestDelegate:self];
}
